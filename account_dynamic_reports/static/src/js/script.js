odoo.define('account_dynamic_reports.DynamicTbMain', function (require) {
        ...
        fetch_move_lines_by_page: function (event) {
            ...
            self.ageing_lines_by_page(offset, partner_id).then(function (datas) {
                ...
                _.each(account_data, function (k, v) {
                    ...
                    k.balance = self.formatWithSign(k.balance, formatOptions, k.balance < 0 ? '-' : '');
                    k.residual = self.formatWithSign(k.residual, formatOptions, k.residual < 0 ? '-' : '');
    ...
    var DynamicPiMain = AbstractAction.extend({
        template: 'DynamicPiMain',
        events: {
            'click #filter_apply_button': 'update_with_filter',
            'click #pdf': 'print_pdf',
            'click #xlsx': 'print_xlsx',
        ...
        print_pdf: function (e) {
            ...
            self._rpc({
                ...
                method: 'get_report_datas',
                args: [
                    self.wizard_id, true, true
                ],
            }).then(function (data) {
                var action = {
                    'type': 'ir.actions.report',
                    'report_type': 'qweb-pdf',
                    'report_name': 'account_dynamic_reports.partner_invoice',
                    'report_file': 'account_dynamic_reports.partner_invoice',
                    'data': {
                        'js_data': data
                    },
                    'context': {
                        'active_model': 'ins.partner.invoice',
                        'landscape': 1,
                        'from_js': true
                    },
                    'display_name': 'Partner Invoices',
                };
                return self.do_action(action);
            });
        },
        ...
        plot_data: function (initial_render = true) {
            ...
            self._rpc({
                ...
                _.each(self.account_data, function (k, v) {
                    ...
                    k.amount_currency = self.formatWithSign(k.amount_currency, formatOptions, k.amount_currency < 0 ? '-' : '');
                    k.amount_unreconciled = self.formatWithSign(k.amount_unreconciled, formatOptions, k.amount_unreconciled < 0 ? '-' : '');

                    k.date_invoice = field_utils.format.date(field_utils.parse.date(k.date_invoice, {}, {
                        isUTC: true
                    }));
                });
                if (initial_render) {
                    self.$('.py-control-panel').html(QWeb.render('FilterSectionPi', {
                        filter_data: datas[0],
                    }));
                    ...
                    self.$el.find('.invoice-type-multiple').select2({
                        maximumSelectionSize: 1,
                ...
                self.$('.py-data-container-orig').html(QWeb.render('DataSectionPi', {
                    account_data: datas[1]
                }));
                self.loader_enable_ui();
            });
        },
        pl_lines_by_page: function (account_id) {
            var self = this;
            return self._rpc({
                model: 'ins.partner.invoice',
                method: 'build_detailed_move_lines',
        ...
        fetch_move_lines: function (event) {
            ...
            var self = this;
            var account_id = $(event.currentTarget).data('account-id');
            //var internal_type = $(event.currentTarget).data('internal-type');
            var td = $(event.currentTarget).next('tr').find('td');
            if (td.length == 1) {
                self.loader_disable_ui();
                self.pl_lines_by_page(account_id).then(function (datas) {
                    _.each(datas, function (k, v) {
                        var formatOptions = {
                            currency_id: k.company_currency_id,
                            noSymbol: true,
                        };
                        k.doc_amount = self.formatWithSign(k.doc_amount, formatOptions, k.doc_amount < 0 ? '-' : '');
                        k.knock_off_amount = self.formatWithSign(k.knock_off_amount, formatOptions, k.knock_off_amount < 0 ? '-' : '');
                        k.knock_off_in_currency = self.formatWithSign(k.knock_off_in_currency, formatOptions, k.knock_off_in_currency < 0 ? '-' : '');
                        k.date = field_utils.format.date(field_utils.parse.date(k.date, {}, {
                            isUTC: true
                        }));
                    });
                    $(event.currentTarget).next('tr').find('td .py-mline-table-div').remove();
                    $(event.currentTarget).next('tr').find('td ul').after(
                        QWeb.render('SubSectionPi', {
                            account_data: datas,
                        }))
                    $(event.currentTarget).next('tr').find('td ul li:first a').css({
                        'background-color': '#00ede8',
        ...
        view_move_line: function (event) {
            ...
            //method: 'search_read',
            //domain: [
            //    ['move_id', '=', $(event.currentTarget).data('move-id')]
            //],
            //fields: ['id'],
            //limit: 1,
            //})
        ...
        update_with_filter: function (event) {
            ...
            if ($(".invoice-type-multiple").select2('data').length === 1) {
                output.invoice_type = $(".invoice-type-multiple").select2('data')[0].id
            }
            var journal_ids = [];
            var journal_list = $(".journal-multiple").select2('data')
            for (var i = 0; i < journal_list.length; i++) {
    ...
    var DynamicPpMain = AbstractAction.extend({
        template: 'DynamicPpMain',
        events: {
            'click #filter_apply_button': 'update_with_filter',
            'click #pdf': 'print_pdf',
            'click #xlsx': 'print_xlsx',
        ...
        print_pdf: function (e) {
            ...
            self._rpc({
                ...
                var action = {
                    'type': 'ir.actions.report',
                    'report_type': 'qweb-pdf',
                    'report_name': 'account_dynamic_reports.partner_payment',
                    'report_file': 'account_dynamic_reports.partner_payment',
                    'data': {'js_data': data},
                    'context': {
                        'active_model': 'ins.partner.payment',
                        'landscape': 1,
                        'from_js': true
                    },
                    'display_name': 'Partner Payments',
                };
                return self.do_action(action);
            });
        },
        ...
        plot_data: function (initial_render = true) {
            ...
            self._rpc({
                ...
                _.each(self.account_data, function (k, v) {
                    ...
                    k.amount_currency = self.formatWithSign(k.amount_currency, formatOptions, k.amount_currency < 0 ? '-' : '');
                    k.amount_unreconciled = self.formatWithSign(k.amount_unreconciled, formatOptions, k.amount_unreconciled < 0 ? '-' : '');
                    k.payment_date = field_utils.format.date(field_utils.parse.date(k.payment_date, {}, {
                        isUTC: true
                    }));
                });
                if (initial_render) {
                    self.$('.py-control-panel').html(QWeb.render('FilterSectionPp', {
                        filter_data: datas[0],
                    }));
                    self.$el.find('#date_from').datepicker({
                        dateFormat: 'dd-mm-yy'
                    ...
                    self.$el.find('.payment-type-multiple').select2({
                        maximumSelectionSize: 1,
                ...
                self.$('.py-data-container-orig').html(QWeb.render('DataSectionPp', {
                    account_data: datas[1]
                }));
                self.loader_enable_ui();
            });
        },
        pl_lines_by_page: function (account_id, internal_type) {
            var self = this;
            return self._rpc({
                model: 'ins.partner.payment',
                method: 'build_detailed_move_lines',
                args: [self.wizard_id, account_id, internal_type],
            })
        },
        ...
        fetch_move_lines: function (event) {
            ...
            if (td.length == 1) {
                self.loader_disable_ui();
                self.pl_lines_by_page(account_id, internal_type).then(function (datas) {
                    _.each(datas, function (k, v) {
                        var formatOptions = {
                            currency_id: k.company_currency_id,
                            noSymbol: true,
        ...
        update_with_filter: function (event) {
            ...
            if ($(".payment-type-multiple").select2('data').length === 1) {
                output.payment_type = $(".payment-type-multiple").select2('data')[0].id
            }
            var journal_ids = [];
            var journal_list = $(".journal-multiple").select2('data')
            for (var i = 0; i < journal_list.length; i++) {
    ...
    }); core.action_registry.add('dynamic.fr', DynamicFrMain); core.action_registry.add('dynamic.gl', DynamicGlMain); core.action_registry.add('dynamic.pa', DynamicPaMain); core.action_registry.add('dynamic.pl', DynamicPlMain); core.action_registry.add('dynamic.tb', DynamicTbMain); core.action_registry.add('dynamic.pi', DynamicPiMain);core.action_registry.add('dynamic.pp', DynamicPpMain);
});
