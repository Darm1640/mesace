...
class as_assets_report_excel(models.TransientModel):
    ...
    def imprimir_excel(self):
        ...
        datas['model'] = 'as.report.factura.excel'
        datas['form'] = self.read()[0]
        for field in datas['form'].keys():
            if isinstance(datas['form'][field], tuple):
                datas['form'][field] = datas['form'][field][0]
        if context.get('xlsx_export'):
            return self.env.ref('as_spectrocom_sales.reporte_siat_xlsx').report_action(self, data=datas)
