...
class hr_closing_configuration_detail(models.Model):
    ...
    process_id = fields.Many2one('hr.closing.configuration.header', string = 'Proceso', tracking=True)
    department = fields.Many2one('hr.department', string = 'Departamento', tracking=True)
    company = fields.Many2one('res.company', string = 'Compañía', tracking=True, required=True)
