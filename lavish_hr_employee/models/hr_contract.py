...
class hr_contract_concepts(models.Model):
    ...
    input_id = fields.Many2one('hr.salary.rule', 'Regla', required=True, help='Regla salarial', domain="[('types_employee','in',[type_employee])]", tracking=True)
    ...
    period = fields.Selection([('limited', 'Limitado'), ('indefinite', 'Indefinido')], 'Periodo', required=True, tracking=True)
    amount = fields.Float('Valor', help='Valor de la cuota o porcentaje segun formula de la regla salarial', required=True, tracking=True)
    
    aplicar = fields.Selection([('15','Primera quincena'),
                                ('30','Segunda quincena'),
                                ('0','Siempre')],'Aplicar cobro',  required=True, help='Indica a que quincena se va a aplicar la deduccion', tracking=True)
    date_start = fields.Date('Fecha Inicial', tracking=True)
    date_end = fields.Date('Fecha Final')
    partner_id = fields.Many2one('hr.employee.entities', 'Entidad', tracking=True)
    contract_id = fields.Many2one('hr.contract', 'Contrato', required=True, ondelete='cascade')
    ...
    state = fields.Selection([('draft', 'Por Aprobar'),
                              ('done', 'Aprobado'),
                              ('cancel', 'Cancelado / Finalizado')], string='Estado', default='draft', required=True, tracking=True)
...
class hr_contract(models.Model):
    ...
    #branch_id = fields.Many2one('lavish.res.branch', string='Sucursal', related='analytic_account_id.branch_id', readonly=True)
...
class hr_labor_certificate_history(models.Model):
    ...
    def generate_report(self):
        ...
        pdf = self.env.ref('lavish_hr_employee.report_certificacion_laboral_action',False)._render_qweb_pdf(self.id)[0]
