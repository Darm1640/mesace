...
class hr_employee(models.Model):
    ...
    def _get_default_type_thirdparty(self):
        id_type = self.env['lavish.type_thirdparty'].search([('types', '=', '4')], limit=1).id
        if id_type:
            return id_type
        else:
            return False
    ...
    type_thirdparty = fields.Many2one('lavish.type_thirdparty',string='Tipo de tercero', domain=[('types', '=', '4')], default=_get_default_type_thirdparty)
    ...
    certificate = fields.Selection(selection_add=[('primary','Primaria'),
                                    ...
                                    ('magister','Maestría')],
                                    string='Nivel de certificado', tracking=True)
