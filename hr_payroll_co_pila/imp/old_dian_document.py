...
class DianDocument(models.Model):
    ...
    document_id = fields.Many2one(
        "account.move", string="Número de documento", readonly=True, required=True
    )
    ...
    transaction_code = fields.Integer(
        string="Código de la Transacción de validación", readonly=True
    )
    transaction_description = fields.Char(
        string="Descripción de la transacción de validación", readonly=True
    )
    ...
    xml_response_contingency_dian = fields.Text(
        string="Mensaje de respuesta DIAN al envío de la contigencia", readonly=True
    )
    ...
    def _get_resolution_dian(self, data_header_doc):
        ...
        rec_active_resolution = data_header_doc.journal_id.sequence_id.dian_resolution_ids.filtered(
        ...
        if not dic_result_verify_status["result_verify_status"]:
            ...
            if document_type == "c":
                ...
                for by_validate_cn in by_validate_credit_notes:
                    ...
                    if invoice_validated:
                        ...
                        cn_with_validated_invoices_ids.append(by_validate_cn.id)
                        if not self.document_id.cufe_cuds_other_system:
                            raise UserError(
                                _(
            ...
            for doc_send_dian in docs_send_dian:
                ...
                if data_constants_document["InvoiceTypeCode"] in (
                    ...
                    data_taxs_xml = self._generate_taxs_data_xml(
                        template_tax_data_xml,
                        data_taxs,
                        data_constants_document["CurrencyID"],
                        document_type,
                    )
                    # Genera líneas de detalle de las factura
                    data_lines_xml = self._generate_lines_data_xml(
                    ...
                    CUFE = self._generate_cufe(
                        data_header_doc.id,
                        data_constants_document["InvoiceID"],
                        data_constants_document["IssueDateCufe"],
                        data_constants_document["IssueTime"],
                        data_constants_document["LineExtensionAmount"],
                        dian_constants["SupplierID"],
                        data_constants_document["CustomerSchemeID"],
                        data_constants_document["CustomerID"],
                        data_constants_document["TechnicalKey"],
                        data_constants_document["PayableAmount"],
                        data_taxs,
                        ambiente,
                    )
                ...
                if data_constants_document["InvoiceTypeCode"] in ("91", "95"):
                    ...
                    data_credit_lines_xml = self._generate_credit_lines_data_xml(
                    ...
                    _logger.warning("****** Generamos CUFE: {}".format(CUFE))
                    ...
                    _logger.warning(
                        "****** Generamos data_xml_document: {}".format(
                            data_xml_document
                        )
                    )
                ...
                if data_constants_document["InvoiceTypeCode"] == "92":
                    ...
                    # Detalle líneas de nota de crédito
                    data_debit_lines_xml = self._generate_debit_lines_data_xml(
                    ...
                    if response.status_code != 200:  # Respuesta de envío no exitosa
                        ...
                        if company.production:
                            ...
                            if (
                                ...
                                data_header_doc.write({"diancode_id": doc_send_dian.id})
                                # Generar código QR
                                doc_send_dian.QR_code = self.sudo()._generate_barcode(
                                    dian_constants,
                                    data_constants_document,
                                    CUFE,
                                    data_taxs,
                            ...
                            # Generar código QR
                            doc_send_dian.QR_code = self.sudo()._generate_barcode(
                                dian_constants, data_constants_document, CUFE, data_taxs
                            )
    ...
    def enviar_email(self, data_xml_document, invoice_id, fileName):
        ...
        pdf = (
            ...
            .sudo()
            ._render_qweb_pdf([rs_invoice.id])[0]
        )
        ...
        for adj in adjuntos:
            if adj.name[::-1][0:3] == "piz":
                continue
            if adj.name not in [
                f"Factura_{rs_invoice.name}.pdf",
                fileName,
        ...
        zip_file = self._generate_zip_multiple_files(
            [(f"Factura_{rs_invoice.name}.pdf", pdf)]
            + [
    ...
    def generate_attached_document(
        ...
        data_xml_document = self._template_attached_document() % {
            ...
            "UUID": cufe,
            "ApplicationResponse": application_response.replace("\r", "").replace(
                "\n", ""
            ),
    ...
    def get_pem(self):
        ...
        try:
            ...
            return crypto.load_certificate(crypto.FILETYPE_PEM, archivo_pem)
        except Exception as ex:
            raise UserError(tools.ustr(ex))
    ...
    def _generate_SignatureValue_GetStatus(self, data_xml_SignedInfo_generate):
        ...
        pem = self.get_pem()
        try:
            crypto.verify(pem, signature, data_xml_SignatureValue_c14n, "sha256")
        except Exception:
    ...
    def _get_dian_constants(self, data_header_doc):
        ...
        dian_constants["SoftwareProviderID"] = (
            company.partner_id.vat if company.partner_id.vat else ""
        )
        dian_constants["SoftwareProviderSchemeID"] = (
            company.partner_id.dv if company.partner_id.dv else 0
        )

        dian_constants["ProviderID"] = (
            partner.vat if partner.vat else ""
        )  # ID Proveedor de software o cliente si es software propio
        dian_constants[
            "SoftwareID"
        ] = self._get_software_identification_code()  # ID del software a utilizar
        ...
        dian_constants[
            "UBLVersionID"
        ] = "UBL 2.1"  # Versión base de UBL usada. Debe marcar UBL 2.0
        ...
        dian_constants[
            "SupplierDepartment"
        ] = partner.state_id.name  # Ciudad o departamento (No requerido)
        dian_constants[
            "SupplierCityCode"
        ] = partner.xcity.code  # Municipio tabla 6.4.3 res.country.state.city
        dian_constants[
            "SupplierCityName"
        ] = partner.xcity.name.title()  # Municipio tabla 6.4.3 res.country.state.city
        dian_constants[
            "SupplierCountrySubentity"
        ] = partner.state_id.name  # Ciudad o departamento tabla 6.4.2 res.country.state
        if not partner.xcity:
            raise UserError(
                _("El partner {} no tiene ciudad establecida".format(partner.name))
            )
        dian_constants["SupplierCountrySubentityCode"] = partner.xcity.code[
            0:2
        ...
        dian_constants["NitSinDV"] = partner.vat
    ...
    def _generate_data_constants_document(
        ...
        data_constants_document["InvoiceAuthorization"] = data_resolution[
            "InvoiceAuthorization"
        ...
        data_constants_document[
            ...
            data_header_doc.amount_untaxed
        )  # Total Importe bruto antes de impuestos: Total importe bruto, suma de los importes brutos de las líneas de la factura.
        # Valor bruto más tributos
        data_constants_document[
            "TotalTaxInclusiveAmount"
        ] = self._complements_second_decimal_total(
            data_header_doc.amount_without_wh_tax
        )

        data_constants_document[
            "TaxExclusiveAmount"
        ] = self._caculate_TaxExclusiveAmount(data_header_doc.id)

        # data_constants_document['TotalTaxInclusiveAmount'] = self._complements_second_decimal_total(data_header_doc.amount_total)
        data_constants_document[
            "TaxExclusiveAmount"
        ] = self._complements_second_decimal_total(
            data_constants_document["TaxExclusiveAmount"]
        ...
        data_constants_document[
            ...
            data_header_doc.amount_without_wh_tax
        )  # Total de Factura: Total importe bruto + Total Impuestos-Total Impuesto Retenidos

        date_invoice_cufe = self._generate_datetime_IssueDate()
        data_constants_document["IssueDate"] = date_invoice_cufe[
            "IssueDate"
        ...
        data_constants_document[
            "IssueTime"
        ] = self._get_time_colombia()  # Hora de emisión de la fcatura
        data_constants_document["InvoiceTypeCode"] = self._get_doctype(
            data_header_doc.move_type, data_header_doc.debit_origin_id, in_contingency_4
        )
        data_constants_document["CreditNoteTypeCode"] = self._get_doctype(
            data_header_doc.move_type, data_header_doc.debit_origin_id, in_contingency_4
        )
        ...
        if (
            ...
            data_constants_document["IDAdquiriente"] = (
                partner_to_send.vat if partner_to_send.vat else ""
            )
            data_constants_document[
                "SchemeNameAdquiriente"
            ] = self.return_number_document_type(partner_to_send.l10n_co_document_code)
            data_constants_document["SchemeIDAdquiriente"] = partner_to_send.dv
        else:
            data_constants_document["IDAdquiriente"] = (
                partner_to_send.vat if partner_to_send.vat else ""
            )
        ...
        data_constants_document[
            "CustomerCityCode"
        ] = partner_to_send.xcity.code  # Municipio tabla 6.4.3 res.country.state.city
        data_constants_document[
            "CustomerCityName"
        ] = (
            partner_to_send.xcity.name.title()
        ...
        ] = partner_to_send.xcity.code[
            0:2
        ]  # Ciudad o departamento tabla 6.4.2 res.country.state
        data_constants_document[
            "CustomerCountryCode"
        ] = partner_to_send.country_id.code  # País tabla 6.4.1 res.country
        data_constants_document[
            "CustomerCountryName"
        ...
        ] = self._replace_character_especial(partner_to_send.companyName)
        ...
        data_constants_document[
            "CustomerschemeID"
        ] = partner_to_send.dv  # Digito verificador del NIT
        data_constants_document["Firstname"] = self._replace_character_especial(
            partner_to_send.name
        )
    ...
    def _caculate_TaxExclusiveAmount(self, invoice_id):
        """
            Funcion que retorna el amount del impuesto
        """
        amount_untaxed = 0.00
        data_lines_doc = self.env["account.move.line"].search(
            [
                ("move_id", "=", invoice_id),
                ("product_id", "!=", None),
        ...
        for data_line_doc in data_lines_doc:
            if data_line_doc.tax_ids:
                if not sum(
                    data_line_doc.tax_ids.filtered(
                        lambda x: x.tributes not in ("05", "06", "07", "08")
                    ).mapped("amount")
                ):
                    continue
...
<CreditNote xmlns="urn:oasis:names:specification:ubl:schema:xsd:CreditNote-2" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2" xmlns:ds="http://www.w3.org/2000/09/xmldsig#" xmlns:ext="urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2" xmlns:sts="http://www.dian.gov.co/contratos/facturaelectronica/v1/Structures" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="urn:oasis:names:specification:ubl:schema:xsd:CreditNote-2    http://docs.oasis-open.org/ubl/os-UBL-2.1/xsd/maindoc/UBL-CreditNote-2.1.xsd">
    ...
    def _generate_data_nc_document_xml(
        ...
        _logger.warning("***** dcd: {}".format(dcd))
...
<rep:Document>%(Document)s</rep:Document>
    ...
    def _get_taxs_data(self, invoice_id):
        ...
        for line in invoice.invoice_line_ids:
            for tax in line.tax_ids:
                ...
                group = tax.tax_group_fe
                if group == "nap_fe":
                    continue

                per = tax.amount if tax.amount > 0 else tax.amount * -1
                pro = (
                    (
                        line.price_subtotal
                ...
                prefix = group.replace("_fe", "")

                counter.update({group: counter.get(group, 0) + 1})
                lines = counter.get(group)

                dic_taxs_data[
                    prefix + "_" + str(lines)
                ] = self._complements_second_decimal_total(pro * -1 if pro < 0 else pro)

                dic_taxs_data[prefix + "_total_general"] = dic_taxs_data.get(
                    prefix + "_total_general", 0
                ) + (pro * -1 if pro < 0 else pro)

                dic_taxs_data[
                    "tax_percentage_" + prefix + "_" + str(lines)
                ] = self._complements_second_decimal_total(
                    tax.amount if tax.amount > 0 else tax.amount * -1, True
                )

                dic_taxs_data[
                    "total_base_" + prefix + "_" + str(lines)
                ] = self._complements_second_decimal_total(
                    line.price_subtotal
                    if line.price_subtotal > 0
                    else line.price_subtotal * -1
                ...
                dic_taxs_data[prefix + "_lines"] = lines
    ...
    def _generate_taxs_data_xml(self, template_tax_data_xml, data_taxs, CurrencyID, document_type):
        ...
        for iva_line in range(data_taxs.get("iva_lines", 1)):
            ...
            if data_taxs.get("iva_" + iva_line_str, "0.00") != "0.00":
                SumTaxTotalTaxAmount += float(
                    data_taxs["iva_" + iva_line_str]
                )  # Importe Impuesto (detalle): Importe del impuesto retenido
                TaxTotalTaxAmount = str(data_taxs["iva_" + iva_line_str])
                TaxTotalTaxableAmount = str(
                    data_taxs["total_base_iva_" + iva_line_str]
                )  # 7.1.1.1 / 8.1.1.1 - Base Imponible: Base Imponible sobre la que se calcula la retención de impuesto
                TaxTotalPercent = str(
                    data_taxs["tax_percentage_iva_" + iva_line_str]
                )  # 7.1.1.3 / 8.1.1.3 - Porcentaje: Porcentaje a aplicar
                ...
                data_tax_xml += template_tax_data_xml % {
        ...
        if SumTaxTotalTaxAmount:
            data_tax_xml = """
                <cac:TaxTotal>
                    <cbc:TaxAmount currencyID="{CurrencyID}">{TaxTotalTaxAmount}</cbc:TaxAmount>
                    <cbc:RoundingAmount currencyID="{CurrencyID}">{TaxRoundingAmount}</cbc:RoundingAmount>
                    {Taxes}
                </cac:TaxTotal>
                """.format(
                ...
                TaxTotalTaxAmount=self._complements_second_decimal_total(
                    SumTaxTotalTaxAmount
                ),
                CurrencyID=CurrencyID,
                Taxes=data_tax_xml,
            )
        ...
        TaxRoundingAmount = 0
        data_tax_xml_ica = ""
        for ica_line in range(data_taxs.get("ica_lines", 1)):
            ica_line_str = str(ica_line + 1)
            if data_taxs.get("ica_" + ica_line_str, "0.00") != "0.00":
                SumTaxTotalTaxAmount += float(
                    data_taxs["ica_" + ica_line_str]
                )  # Importe Impuesto (detalle): Importe del impuesto retenido
                TaxTotalTaxAmount = str(data_taxs["ica_" + ica_line_str])
                TaxTotalTaxableAmount = str(
                    data_taxs["total_base_ica_" + ica_line_str]
                )  # 7.1.1.1 / 8.1.1.1 - Base Imponible: Base Imponible sobre la que se calcula la retención de impuesto
                TaxTotalPercent = str(
                    data_taxs["tax_percentage_ica_" + ica_line_str]
                )  # 7.1.1.3 / 8.1.1.3 - Porcentaje: Porcentaje a aplicar
                ...
                imp_2 = float(data_taxs["total_base_ica_" + ica_line_str])
                imp_6 = float(data_taxs["tax_percentage_ica_" + ica_line_str])
                imp_4 = imp_2 * imp_6 / 100
                imp_6_imp_2 = (imp_2 * imp_6) / 100
                TaxRoundingAmount += imp_6_imp_2 - imp_4

                data_tax_xml_ica += template_tax_data_xml % {
                    "TaxTotalTaxAmount": TaxTotalTaxAmount,
                    # 'TaxTotalTaxEvidenceIndicator' : TaxTotalTaxEvidenceIndicator,
                    "TaxTotalTaxableAmount": TaxTotalTaxableAmount,
                    "TaxTotalPercent": TaxTotalPercent,
        ...
        TaxRoundingAmount = 0
        data_tax_xml_inc = ""
        for inc_line in range(data_taxs.get("ico_lines", 1)):
            inc_line_str = str(inc_line + 1)
            if data_taxs.get("ico_" + inc_line_str, "0.00") != "0.00":
                SumTaxTotalTaxAmount += float(
                    data_taxs["ico_" + inc_line_str]
                )  # Importe Impuesto (detalle): Importe del impuesto retenido
                TaxTotalTaxAmount = str(data_taxs["ico_" + inc_line_str])
                TaxTotalTaxableAmount = str(
                    data_taxs["total_base_ico_" + inc_line_str]
                )  # 7.1.1.1 / 8.1.1.1 - Base Imponible: Base Imponible sobre la que se calcula la retención de impuesto
                TaxTotalPercent = str(
                    data_taxs["tax_percentage_ico_" + inc_line_str]
                )  # 7.1.1.3 / 8.1.1.3 - Porcentaje: Porcentaje a aplincr
                TaxTotalTaxSchemeID = "04"  # 7.1.1.2 - Tipo: Tipo o clase impuesto. Concepto fiscal por el que se tributa. Debería si un campo que referencia a una lista de códigos. En la lista deberían aparecer los impuestos estatales o nacionales. Código de impuesto
                TaxTotalName = "INC"

                imp_2 = float(data_taxs["total_base_ico_" + inc_line_str])
                imp_6 = float(data_taxs["tax_percentage_ico_" + inc_line_str])
                imp_4 = imp_2 * imp_6 / 100
                imp_6_imp_2 = (imp_2 * imp_6) / 100
                TaxRoundingAmount += imp_6_imp_2 - imp_4

                data_tax_xml_inc += template_tax_data_xml % {
                    "TaxTotalTaxAmount": TaxTotalTaxAmount,
                    # 'TaxTotalTaxEvidenceIndinctor' : TaxTotalTaxEvidenceIndinctor,
                    "TaxTotalTaxableAmount": TaxTotalTaxableAmount,
                    "TaxTotalPercent": TaxTotalPercent,
                    "TaxTotalName": TaxTotalName,
                    "TaxTotalTaxSchemeID": TaxTotalTaxSchemeID,
        ...
        for ret_line in range(data_taxs.get("ret_lines", 1)):
            ...
            if data_taxs.get("ret_" + ret_line_str, "0.00") != "0.00":
                SumTaxTotalTaxAmount += float(
                    data_taxs["ret_" + ret_line_str]
                )  # Importe Impuesto (detalle): Importe del impuesto retenido
                TaxTotalTaxAmount = str(data_taxs["ret_" + ret_line_str])
                TaxTotalTaxableAmount = str(
                    data_taxs["total_base_ret_" + ret_line_str]
                )  # 7.1.1.1 / 8.1.1.1 - Base Imponible: Base Imponible sobre la que se calcula la retención de impuesto
                TaxTotalPercent = str(
                    data_taxs["tax_percentage_ret_" + ret_line_str]
                )  # 7.1.1.3 / 8.1.1.3 - Porcentaje: Porcentaje a aplicar
                ...
                data_tax_xml_ret += template_tax_data_xml % {
                    "TaxTotalTaxAmount": TaxTotalTaxAmount,
                    # 'TaxTotalTaxEvidenceIndicator' : TaxTotalTaxEvidenceIndicator,
                    "TaxTotalTaxableAmount": TaxTotalTaxableAmount,
                    "TaxTotalPercent": TaxTotalPercent,
    ...
    def _generate_lines_data_xml(self, template_line_data_xml, invoice_id, CurrencyID):
        ...
        for data_line in data_lines_doc:
            ...
            if data_line.price_subtotal:
                ILLinea += 1
                ILInvoicedQuantity = self._complements_second_decimal_total(
                    data_line.quantity
                )  # 13.1.1.9 - Cantidad: Cantidad del artículo solicitado. Número de unidades servidas/prestadas.
                ILLineExtensionAmount = self._complements_second_decimal_total(
                    data_line.price_subtotal
                ...
                ILAmount = self._complements_second_decimal_total(
                    data_line.discount
                )  # Valor Descuento: Importe total a descontar.
                ILDescription = self._replace_character_especial(data_line.name)
                ILPriceAmount = self._complements_second_decimal_total(
                    data_line.price_unit
                )  # Precio Unitario
                BrandName = self._replace_character_especial(
                    data_line.product_id.brand_id.name
                    if data_line.product_id.brand_id
                    else ""
                )
                ModelName = self._replace_character_especial(
                    data_line.product_id.model_id.name
                    if data_line.product_id.model_id
                    else ""
                )
                ...
                TaxRoundingAmount = 0
                for line_tax in data_line.tax_ids.filtered(lambda x: x.tax_group_fe == "iva_fe"):
                    tax = self.env["account.tax"].search([("id", "=", line_tax.id)])
                    # ILTaxAmount += data_line.price_subtotal * (tax.amount / 100.00)
                    price_total_without_ret = (
                        data_line.price_subtotal * tax.amount
                    ) / 100
                    ILTaxAmount += price_total_without_ret
                    ILTaxAmount = float(
                        self._complements_second_decimal_total(
                            ILTaxAmount * -1 if ILTaxAmount < 0 else ILTaxAmount
                        )
                    )
                    ILTaxAmountSubtotal = (
                        data_line.price_subtotal * tax.amount
                    ...
                    ILTaxAmountSubtotal = self._complements_second_decimal_total(
                        ILTaxAmountSubtotal * -1
                        if ILTaxAmountSubtotal < 0
                        else ILTaxAmountSubtotal
                    )
                    if data_line.tax_ids.filtered(
                        lambda x: x.tax_group_fe in ("ret_fe", "ica_fe")
                    ):
                        ILTaxAmountSubtotal = self._complements_second_decimal_total(
                            price_total_without_ret
                        )
                    ILTaxableAmount = self._complements_second_decimal_total(
                        data_line.price_subtotal * -1
                        if data_line.price_subtotal < 0
                        else data_line.price_subtotal
                    )
                    ILPercent = self._complements_second_decimal_total(
                        tax.amount * -1 if tax.amount < 0 else tax.amount
                    )
                    ILID = tax.tributes
                    if not tax.tributes:
                        raise UserError(
                            _(
                                "Impuesto {} no tiene establecido tributo dian".format(
                                    tax.name
                                )
                            )
                        )
                ...
                for line_tax in data_line.tax_ids.filtered(lambda x: x.tax_group_fe == "ico_fe"):
                    tax = self.env["account.tax"].search([("id", "=", line_tax.id)])
                    # ILTaxAmount += data_line.price_subtotal * (tax.amount / 100.00)
                    price_total_without_ret = (
                        data_line.price_subtotal * tax.amount
                ...
                if data_line.product_id.product_tmpl_id.operation_type == "11":
                    InformationContentProviderParty = (
                        self._template_line_data_information_content_provider_party_xml()
                    )

                    if not data_line.invoice_id.mandante_id:
                        raise UserError(
                            _(
                                "El Propietario o Mandante no se encuentra seleccionado, en la pestaña otra información puede seleccionarlo"
                            )
                        )
                    ...
                    InformationContentProviderParty = (
                        InformationContentProviderParty
                        % {
                            "MandanteNumberIdentification": data_line.move_id.mandante_id.vat,
                        }
                    )
                # Consultamos si es factura de proveedor de ser asi agregamos los campos correspondiente para Documento Soporte al XML
                if self.document_id.move_type == "in_invoice":
                    InvoicePeriodStartDate = str(data_line.purchase_date)
                    InvoicePeriod = data_line.forma_generacion_ds
                    InvoiceLineNote = data_line.note_ds or "Sin Nota"
                    if not InvoicePeriod:
                        raise UserError(
                            _(
                                "Forma de generacion es obligatorio en las lineas de la factura de proveedor"
                            )
                        )
                    if InvoicePeriod == "1":
                        InvoicePeriodDescription = "Por operación"
                        InvoicePeriodStartDate = self._generate_datetime_IssueDate().get(
                            "IssueDate"
                        )
                    elif InvoicePeriod == "2":
                        InvoicePeriodDescription = "Acumulado semanal"

                    data_line_xml += template_line_data_xml % {
                        "ILLinea": ILLinea,
                        ...
                        "InvoicePeriodDescription": InvoicePeriodDescription,
    ...
    def _generate_credit_lines_data_xml(
        self, template_credit_line_data_xml, invoice_id, CurrencyID
    ):
        ILLinea = 0
        ...
        template_credit_line_without_taxtotal = """
        <cac:CreditNoteLine>
            <cbc:ID>%(ILLinea)s</cbc:ID>
            <cbc:CreditedQuantity unitCode="EA">%(ILInvoicedQuantity)s</cbc:CreditedQuantity>
            <cbc:LineExtensionAmount currencyID="%(CurrencyID)s">%(ILLineExtensionAmount)s</cbc:LineExtensionAmount>
        ...
        for data_line in data_lines_doc:
            ...
            for line_tax in data_line.tax_ids.filtered(lambda x: x.tax_group_fe == "iva_fe"):
                ...
                # price_total_without_ret = (
                #     data_line.price_total - data_line.price_subtotal
                # )
                # if data_line.tax_ids.filtered(lambda x: x.tax_group_fe == "ret_fe"):
                #     price_total_without_ret = data_line.price_unit * tax.amount / 100

                ILTaxAmount = (data_line.price_subtotal * tax.amount) / 100.00

                ILTaxAmount = self._complements_second_decimal_total(
                    ILTaxAmount * -1 if ILTaxAmount < 0 else ILTaxAmount
                )
                # if data_line.tax_ids.filtered(lambda x: x.tax_group_fe == "ret_fe"):
                #     ILTaxAmount = price_total_without_ret
            ...
            if data_line.product_id.product_tmpl_id.operation_type == "11":
                ...
                if not data_line.invoice_id.mandante_id:
                    raise UserError(
                        _(
                            "El Propietario o Mandante no se encuentra seleccionado, en la pestaña otra información "
                            "puede seleccionarlo "
                        )
                    )
            ...
            data_credit_note_line_xml += (
                template_credit_line_without_taxtotal
                if ILID == "06" or ILPercent == "0.00"
                else template_credit_line_data_xml
            ) % {
                "ILLinea": ILLinea,
                "ILInvoicedQuantity": ILInvoicedQuantity,
                "ILLineExtensionAmount": ILLineExtensionAmount,
                # Ojo descuentos mas recargos de la linea
                "ILAmount": ILAmount,
                "ILDescription": ILDescription,
                "BrandName": BrandName,
                "ModelName": ModelName,
                "ILPriceAmount": ILPriceAmount,
                "ILChargeIndicator": ILChargeIndicator,
                "ILTaxAmount": abs(float(ILTaxAmount)),
                "ILTaxableAmount": abs(float(ILTaxableAmount)),
                "ILPercent": ILPercent,
                "ILID": ILID,
                "ILName": ILName,
                "InformationContentProviderParty": InformationContentProviderParty,
    ...
    def _generate_debit_lines_data_xml(
        self, template_debit_line_data_xml, invoice_id, CurrencyID
    ):
        ILLinea = 0
        ...
        for data_line in data_lines_doc:
            ...
            for line_tax in data_line.tax_ids.filtered(lambda x: x.tax_group_fe == "iva_fe"):
                ...
                ILTaxAmount = self._complements_second_decimal_total(
                    (data_line.price_subtotal * tax.amount) / 100.00
                )
                ILTaxableAmount = self._complements_second_decimal_total(
                    data_line.price_subtotal
                )
            ...
            data_debit_note_line_xml += template_debit_line_data_xml % {
                "ILLinea": ILLinea,
                "ILInvoicedQuantity": ILInvoicedQuantity,
                "ILLineExtensionAmount": ILLineExtensionAmount,  # Ojo descuentos mas recargos de la linea
                "ILAmount": ILAmount,
    ...
    def _generate_cufe(
        ...
        CodImp2 = "04"
        ValImp2 = 0
        if data_taxs.get("ico_lines"):
            for inc_line in range(data_taxs.get("ico_lines")):
                inc_line_str = str(inc_line + 1)
                ValImp2 += float(data_taxs["ico_" + inc_line_str])        
        ValImp2 = str(self._complements_second_decimal_total(ValImp2))
        CodImp3 = "03"
        ValImp3 = 0
        if data_taxs.get("ica_lines"):
            for ica_line in range(data_taxs.get("ica_lines")):
                ica_line_str = str(ica_line + 1)
                ValImp3 += float(data_taxs["ica_" + ica_line_str])        
        ValImp3 = str(self._complements_second_decimal_total(ValImp3))        
        ValPag = str(ValPag)
        TipAdq = str(TipAdq)
        CUFE = (
            NumFac
    ...
    def _generate_cude(
        ...
        _logger.warning("**** CUDE: {}".format(CUDE))
    ...
    def _generate_CertDigestDigestValue(self):
        ...
        certificate = hashlib.sha256(
            crypto.dump_certificate(crypto.FILETYPE_ASN1, key.get_certificate())
        )
    ...
    def _generate_SignatureValue(self, data_xml_SignedInfo_generate):
        ...
        try:
            ...
            raise UserError(_("Firma no fué validada exitosamente"))
    ...
    def _generate_barcode(
        ...
        ValOtroIm = str(data_taxs.get("ico_04", 0) + data_taxs.get("ica_03", 0))
...
"""
    ...
    # wsu:Id="TS-%(identifier)s"> <wsu:Created>%(Created)s</wsu:Created> <wsu:Expires>%(Expires)s</wsu:Expires>
    # </wsu:Timestamp> <wsse:BinarySecurityToken
    # EncodingType="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-soap-message-security-1.0#Base64Binary"
    # ValueType="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-x509-token-profile-1.0#X509v3"
    # wsu:Id="BAKENDEVS-%(identifierSecurityToken)s">%(Certificate)s</wsse:BinarySecurityToken> <ds:Signature
    # Id="SIG-%(identifier)s" xmlns:ds="http://www.w3.org/2000/09/xmldsig#"> <ds:SignedInfo>
    # <ds:CanonicalizationMethod Algorithm="http://www.w3.org/2001/10/xml-exc-c14n#"> <ec:InclusiveNamespaces
    # PrefixList="wsa soap wcf" xmlns:ec="http://www.w3.org/2001/10/xml-exc-c14n#"/> </ds:CanonicalizationMethod>
    # <ds:SignatureMethod Algorithm="http://www.w3.org/2001/04/xmldsig-more#rsa-sha256"/> <ds:Reference URI="#ID-%(
    # identifierTo)s"> <ds:Transforms> <ds:Transform Algorithm="http://www.w3.org/2001/10/xml-exc-c14n#">
    # <ec:InclusiveNamespaces PrefixList="soap wcf" xmlns:ec="http://www.w3.org/2001/10/xml-exc-c14n#"/>
    # </ds:Transform> </ds:Transforms> <ds:DigestMethod Algorithm="http://www.w3.org/2001/04/xmlenc#sha256"/>
    # <ds:DigestValue></ds:DigestValue> </ds:Reference> </ds:SignedInfo> <ds:SignatureValue></ds:SignatureValue>
    # <ds:KeyInfo Id="KI-%(identifier)s"> <wsse:SecurityTokenReference wsu:Id="STR-%(identifier)s"> <wsse:Reference
    # URI="#BAKENDEVS-%(identifierSecurityToken)s"
    # ValueType="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-x509-token-profile-1.0#X509v3"/>
    # </wsse:SecurityTokenReference> </ds:KeyInfo> </ds:Signature> </wsse:Security>
    # <wsa:Action>http://wcf.dian.colombia/IWcfDianCustomerServices/SendBillSync</wsa:Action> <wsa:To wsu:Id="ID-%(
    # identifierTo)s" xmlns:wsu="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd
    # ">vpfe-hab.dian.gov.co/WcfDianCustomerServices.svc</wsa:To> </soap:Header> <soap:Body> <wcf:SendBillSync>
    # <wcf:fileName>%(fileName)s</wcf:fileName> <wcf:contentFile>%(contentFile)s</wcf:contentFile>
    # </wcf:SendBillSync> </soap:Body> </soap:Envelope> """
