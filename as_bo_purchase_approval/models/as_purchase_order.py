...
class PurchaseOrder(models.Model):
    ...
    def _get_approvals(self):
        for order in self:
            aprobar = False
            monto = order.currency_id._convert(order.amount_total,order.env.user.company_id.currency_id, order.company_id, order.date_order,round=False) 
            usuario = order.env.user.id
            rango_approval = self.env['as.level.approval'].sudo().search([('as_type','=','purchase'),('as_amount_min','<=',monto),('as_amount_max','>=',monto)],order=" id desc",limit=1)
            if rango_approval:
    ...
    def _find_mail_template_send(self, force_confirmation_template=False,as_type=False):
        if as_type == 'to_approval':
            template_id = self.env['ir.model.data'].xmlid_to_res_id('as_bo_purchase_approval.as_template_purchase_email', raise_if_not_found=False)
        else:
            template_id = self.env['ir.model.data'].xmlid_to_res_id('as_bo_purchase_approval.as_template_approval_purchase_email', raise_if_not_found=False)
        return template_id
