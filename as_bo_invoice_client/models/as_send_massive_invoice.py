...
class res_partner(models.Model):
    ...
    def as_create_evento_significativo(self):
        for contingencia in self:
            if contingencia.as_type_mode == 'packet':
                ...
                json = {
                        "codigoPuntoVenta": str(contingencia.as_sucursal.as_office_number),
                        "codigoSucursal":  str(contingencia.as_pdv_id.as_code),
                ...
                respuesta = as_utility.as_process_json('Crear Contingencia',json,self.env.user.id,self.as_codigo_sistema.as_token_ahorasoft,'Crear Contingencia')
    ...
    def as_send_package(self):
        ...
        if self.as_type_mode == 'packet':
            ...
            if comprobacion.days >= 2:
                self.state = 'fuera'
            if not self.as_cont_cafc and self.as_cont_reason.as_code not in ('1','2','3','4'):
                raise UserError(_("No puede enviar un paquete sin CAFC"))
            ...
            json = {
                    "codigoPuntoVenta": str(self.as_sucursal.as_office_number),
                    "codigoSucursal":  str(self.as_pdv_id.as_code),
            ...
            respuesta = as_utility.as_process_json('Recepción Envio Contingencia',json,self.env.user.id,self.as_codigo_sistema.as_token_ahorasoft,'Recepción Envio Contingencia')
            if respuesta[0] and respuesta[1]['success']:
                if 'codigo' in respuesta[1] and 'factura' in respuesta[1]:
            ...
            respuesta = as_utility.as_process_json('Recepción Envio Masivo',json,self.env.user.id,self.as_codigo_sistema.as_token_ahorasoft,'Recepción Envio Masivo')
            if respuesta[0] and respuesta[1]['success']:
                if 'codigo' in respuesta[1] and 'factura' in respuesta[1]:
    ...
    def as_validate_package(self):
        ...
        if self.as_type_mode == 'packet':
            respuesta = as_utility.as_process_json('Validación Envio Contingencia',json,self.env.user.id,self.as_codigo_sistema.as_token_ahorasoft,'Validación Envio Contingencia')
        else:
            respuesta = as_utility.as_process_json('Validación Envio Masivo',json,self.env.user.id,self.as_codigo_sistema.as_token_ahorasoft,'Validación Envio Masivo')

        if respuesta[0] and respuesta[1]['success']:
            if 'codigo' in respuesta[1] and 'factura' in respuesta[1]:
                valores = respuesta[1]['factura']['ns2:validacionRecepcionMasivaFacturaResponse']['RespuestaServicioFacturacion']
                if valores['codigoEstado'] == '908':
                    self.as_package_valid_code = respuesta[1]['codigo']
