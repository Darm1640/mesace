...
class ConfigFE(models.Model):
    _name = 'l10n_co_factura.config_fe'

    config_fe_detail_id = fields.Many2one(
        'l10n_co_factura.config_fe_details',
    ...
    def _check_unique_config_fe_detail_id(self):
        config_fes = self.env['l10n_co_factura.config_fe'].search([])
    ...
    def get_value(self, field_name, obj_id, can_be_null=False):
        ...
        for attribute in attributes:
            ...
            if attribute in dir(obj):
                ...
                if len(obj) > 1:
                    for item in obj:
                        obj_aux.append(getattr(item, attribute)() if is_function else getattr(item, attribute))
                    obj = obj_aux
                else:
        ...
        if not type(obj).__name__ == config_fe.config_fe_detail_id.tipo and config_fe.config_fe_detail_id.tipo != 'list':
            if not can_be_null:
                raise ValidationError(
