...
class IrAttachment(models.Model):
    ...
    def create(self, values):
        if (not 'store_fname' in values) and ('res_model' in values and values['res_model'] == 'l10n_co_factura.fe_archivos_email'):
            values = self._check_contents(values)
            archivos = self.env['l10n_co_factura.fe_archivos_email'].search([('id', '=', values['res_id'])])
