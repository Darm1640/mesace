...
class as_wizard_fromulas_stock(models.Model):
    ...
    wiz_lineas = fields.Many2many('as.transfer.lines.return', string='Lineas')
    ...
    def default_get(self, fields):
        ...
        res['as_location_id'] = so_line_obj.as_location_dest_id.id
        res['as_location_dest_id'] = so_line_obj.as_location_id.id
        dictlinestock = []
    ...
    def as_get_line_wiz(self):
        ...
        if self.start_date and self.end_date:
            for line in self.as_solicitud.lines_ids:
                if (line.as_deliver_date - timedelta(hours = 4)) >= self.start_date and (line.as_deliver_date - timedelta(hours = 4)) <= self.end_date and not line.as_devuelto:
                    vasl={
                    'product_id': line.product_id.id,
                    'lot_id': line.lot_id.id,
                    'product_uom': line.product_uom.id,
    ...
    def as_get_process(self):
        ...
        for lines in self.wiz_lineas:
            ...
            lines.as_solicitud_line.as_dpicking_general_id = picking_general.id
            lines.as_solicitud_line.as_devuelto = True
        ...
        if movimiento_posi_div:
            ...
            for lines in self.wiz_lineas:
                if lines.as_qty_diff > 0:
                    ...
                    lines.as_solicitud_line.as_dpicking_in_id = picking_posi.id
        ...
        if movimiento_nega_div:
            ...
            for lines in self.wiz_lineas:
                if lines.as_qty_diff < 0:
                    ...
                    lines.as_solicitud_line.as_dpicking_out_id = picking_nega.id
        ...
        cant_hecha = len(self.as_solicitud.lines_ids.filtered(lambda sol: sol.as_devuelto == True))
        for pick in self.as_solicitud.as_return_picks:
            picking_ids.append(pick.id)
        self.as_solicitud.as_return_picks = picking_ids


class as_wizard_fromulas_lines(models.Model):
    _name="as.transfer.lines.return"
