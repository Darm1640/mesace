...
class Payment(models.TransientModel):
    ...
    def action_create_payments(self):
        ...
        invoice_ids = self.env[self.env.context.get('active_model')].browse(active_ids)
        ...
        if self.payment_difference==0.0 and invoice_ids and self.env.user.company_id.commission_based_on_payment:
            for inv in invoice_ids:
                if inv.move_type == 'out_invoice':
                    ...
                    analysis_vals = {}
                    related_sale_commissions = sale_commission.search([('user_ids.id','=',inv.user_id.id)])
        ...
        return super(Payment,self).action_create_payments()
...
class Invoice(models.Model):
    ...
    sales_commission_ids = fields.One2many('sale.commission.analysis','move_id',string="Sales Commission")
    show_commission = fields.Boolean(compute='check_show_commission',string="Show Commission")
    ...
    def action_post(self):
        if self.env.user.company_id.commission_based_on_invoice and self.move_type=='out_invoice':
            sale_commission = self.env['sale.commission']
            analysis_vals = {}
