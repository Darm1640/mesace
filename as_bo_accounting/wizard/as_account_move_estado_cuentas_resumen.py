...
class ReporteCuentas(models.TransientModel):
    _name = 'as.resumen.cuentas'

    as_cuentas_proveedor = fields.Many2one('account.account', string='Cuenta', required = True)
    as_nombre_cliente= fields.Many2many('res.partner', string="Proveedor")
    ...
    def export_xls(self):
        ...
        datas['model'] = 'as.resumen.cuentas'
        datas['form'] = self.read()[0]
        for field in datas['form'].keys():
            if isinstance(datas['form'][field], tuple):
                datas['form'][field] = datas['form'][field][0]
        if context.get('xls_export'):
            return self.env.ref('as_bo_accounting.as_resumen_cuentas_xlsx').report_action(self, data=datas)
