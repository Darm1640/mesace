...
class HrPayslipRun(models.Model):
    ...
    def action_validate(self):
        settings_batch_account = self.env['ir.config_parameter'].sudo().get_param(
            'lavish_hr_payroll.module_hr_payroll_batch_account') or False
...
class HrPayslipEmployees(models.TransientModel):
    ...
    #         payslips = self['hr.payslip']
    #         Payslip = self['hr.payslip']
    #         default_values = Payslip.default_get(Payslip.fields_get())
    #         contracts = self['hr.contract'].search([('id', 'in', obj_contracts.ids)])
    #         structure_id = self['hr.payroll.structure'].search([('id', 'in', obj_structure_id.ids)])
    #         payslip_run = self['hr.payslip.run'].search([('id', 'in', obj_payslip_run.ids)])
    #         #try:
    ...
    #             })
    #             payslip = self['hr.payslip'].new(values)
    ...
    #             #self.compute_sheet_thread(item,self.structure_id,payslip_run,i_contracts,)
...
class Hr_Payslip(models.Model):
    ...
    def _get_payslip_lines(self,inherit_vacation=0,inherit_prima=0,inherit_contrato_dev=0,inherit_contrato_ded=0,localdict=None):
        ...
        for leave in work_entries:
            if leave.leave_id and self.credit_note == False:
                es_continuidad = 1
            ...
            if rule._satisfy_condition(localdict):
                ...
                if obj_concept:
                        ...
                        if concept.state == 'done' and date_start_concept <= date_to.date() and date_end_concept >= date_from.date():
                            ...
                            if days_vac > vacation_days_calculate_absences and rule.dev_or_ded == 'deduccion' and not rule.code in ['TOTALDED']:
                                if inherit_vacation != 0:
                                    ...
                                    if cant_fortnight > 1:
                                        amount = amount * cant_fortnight if not rule.modality_value in ['diario','diario_efectivo'] and rule.type_concepts != 'ley' else amount
                    ...
                    if str(rule.amount_python_compute).find('get_overtime') != -1: #Verficiar si la regla utiliza la tabla hr.overtime por ende es un concepto de novedad del menu horas extras
                        ...
                        if (inherit_contrato_dev != 0 or inherit_contrato_ded != 0) and self.settle_payroll_concepts == False and rule.type_concepts != 'ley' and not rule.code in ['TOTALDEV','TOTALDED','NET']:
                            amount, qty, rate = 0,1.0,100
