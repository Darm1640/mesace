...
class AccountPayment(models.Model):
	...
	account_move_payment_ids = fields.Many2many("account.move.line", "account_move_payment_rel", 'moe_line_id','payment_id',
		string="Buscar Otros Documentos", domain="[('parent_state','!=','draft')]")
	# accounts_receivable_ids = fields.Many2many("account.move", "customer_invoice_payment_rel", 'invoice_id', 'payment_id',
	...
	#pasivos = fields.One2many("account.move.line", "account_move_pasivo_rel", 'moe_line_id','payment_id')
	...
	domain_move_lines = fields.Many2many("account.move", 'domain_move_line_pay_rel', string="restriccion de campos", 
                                    compute="_compute_domain_move_line"
                                    )
	...
	def _create_payment_entry_line(self, move):
		...
		for line in self.payment_line_ids:
			...
			# 	counterpart_aml_dict["analytic_tag_ids"] = [(6, 0, line.analytic_tag_ids.ids)]
			# if line.tax_ids:
			# 	counterpart_aml_dict["tax_ids"] = [(6, 0, line.tax_ids.ids)]
			...
			# 	]
			# )
			# counterpart_aml_dict["tag_ids"] = [(6, 0, atrl_ids.mapped("tag_ids").ids)]
			# if line.tax_repartition_line_id:
			# 	counterpart_aml_dict["tag_ids"] = [(6, 0, line.tax_repartition_line_id.tag_ids.ids)]
			# overwrite = self._context.get("overwrite", {})
			# move_line_vals = overwrite.get("L{}".format(line.sequence), {})
			# counterpart_aml_dict.update(move_line_vals)
			# # Use optional account, when amount is negative
			# self._update_account_on_negative(line, counterpart_aml_dict)
			#deje de meter la mano-------------------------------------------------------------------------------------->
	...
	# 	obj_invoice = self.env['account.move'].search(
	# 		['&', '&', ('partner_id', '=', self.partner_id.id), ('payment_state', 'not in', ['paid']),
	# 		('state', 'not in', ['draft', 'cancel']), (
	# 			'move_type', 'in',
	# 			['out_invoice', 'out_refund', 'out_receipt', 'in_invoice', 'in_refund', 'in_receipt'])])
	# 	obj_invoice_line = self.env['account.move.line'].search(
	# 		[('partner_id', '=', self.partner_id.id),('amount_residual','!=', 0),('account_internal_type', 'in',['payable','receivable']),('parent_state', '=', 'posted')])
	# 	print(obj_invoice.ids)
	# 	print(obj_invoice_line.ids)
	# 	invoice_list_ids = obj_invoice.ids
	# 	move_line_list_ids = obj_invoice_line.ids
	# 	#self.debts_to_pay_ids = [(6, 0, invoice_list_ids)]
	# 	self.estado_cuenta_ids = [(6, 0, move_line_list_ids )]
