...
class AccountMoveReversalpayment(models.TransientModel):
    """
    Account move reversal wizard, it cancel an account move by reversing it.
    """
    _name = 'account.move.reversal.payment'
    _description = 'Account Move Reversal'
    _check_company_auto = True

    move_ids = fields.Many2many('account.move', 'account_move_reversal_payment_move', 'reversal_id', 'move_id', domain=[('state', '=', 'posted')])
    new_move_ids = fields.Many2many('account.move', 'account_move_reversal_p_new_move', 'reversal_id', 'new_move_id')
...
#         res = super(AccountMoveReversalpayment, self).default_get(default_fields)
#         ctx = self._context
#         if ctx.get('active_model') == 'account.payment':
#             pay = self.env['account.payment'].browse(self._context.get('active_ids'))
#             payment = self.env['account.payment.detail'].search([('payment_id','=', pay.id)])
            
#             move_ids = self.env['account.move'].search([('id','in', payment.move_id.ids)])
#             #payment_line = move_ids.filtered(lambda l: l.move_type != 'entry').ids or []
...
#                 #res.update({'move_ids' :[(6,0,payment_line)],})
#             return res 
#     # @api.model
