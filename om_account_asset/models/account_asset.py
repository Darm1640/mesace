...
class AccountAssetCategory(models.Model):
    ...
    account_asset_id = fields.Many2one('account.account', string='Asiento Activo', required=True, domain=[('internal_type','=','other'), ('deprecated', '=', False)], help="Account used to record the purchase of the asset at its original price.")
    account_depreciation_id = fields.Many2one('account.account', string='Entradas de depreciación: Asiento Activo', required=True, domain=[('internal_type','=','other'), ('deprecated', '=', False)], help="Account used in the depreciation entries, to decrease the asset value.")
    account_depreciation_expense_id = fields.Many2one('account.account', string='Entradas de depreciación: Expense Account', required=True, domain=[('internal_type','=','other'), ('deprecated', '=', False)], help="Account used in the periodical entries, to record a part of the asset as expense.")
    journal_id = fields.Many2one('account.journal', string='Diario', required=True)
    company_id = fields.Many2one('res.company', string='Compañia', required=True, default=lambda self: self.env['res.company']._company_default_get('account.asset.category'))
    method = fields.Selection([('linear', 'Linear'), ('degressive', 'Degressive')], string='Computation Method', required=True, default='linear',
        help="Choose the method to use to compute the amount of depreciation lines.\n"
            "  * Linear: Calculated on basis of: Gross Value / Número de depreciaciones\n"
            "  * Degressive: Calculated on basis of: Residual Value * Factor degresivo")
    method_number = fields.Integer(string='Número de depreciaciones', default=5, help="The number of depreciations needed to depreciate your asset")
    method_period = fields.Integer(string='Duración del período', default=1, help="State here the time between 2 depreciations, in months", required=True)
    method_progress_factor = fields.Float('Factor degresivo', default=0.3)
    method_time = fields.Selection([('number', 'Number of Entries'), ('end', 'Ending Date')], string='Método de tiempo', required=True, default='number',
    ...
    group_entries = fields.Boolean(string='Group Diario Entries', help="Check this if you want to group the generated entries by categories.")
...
class AccountAssetAsset(models.Model):
    ...
    state = fields.Selection([('draft', 'Borrador'), ('open', 'Abierto'), ('close', 'Cerrado')], 'Status', required=True, copy=False, default='draft',
    ...
    method = fields.Selection([('linear', 'Linear'), ('degressive', 'Degressive')], string='Computation Method', required=True, readonly=True, states={'draft': [('readonly', False)]}, default='linear',
        help="Choose the method to use to compute the amount of depreciation lines.\n  * Linear: Calculated on basis of: Gross Value / Número de depreciaciones\n"
            "  * Degressive: Calculated on basis of: Residual Value * Factor degresivo")
    method_number = fields.Integer(string='Número de depreciaciones', readonly=True, states={'draft': [('readonly', False)]}, default=5, help="The number of depreciations needed to depreciate your asset")
    ...
    method_progress_factor = fields.Float(string='Factor degresivo', readonly=True, default=0.3, states={'draft': [('readonly', False)]})
    value_residual = fields.Float(compute='_amount_residual', digits=0, string='Residual Value')
    method_time = fields.Selection([('number', 'Number of Entries'), ('end', 'Ending Date')], string='Método de tiempo', required=True, readonly=True, default='number', states={'draft': [('readonly', False)]},
        help="Choose the method to use to compute the dates and number of entries.\n"
             "  * Number of Entries: Fix the number of entries and the time between 2 depreciations.\n"
             "  * Ending Date: Choose the time between 2 depreciations and the date the depreciations won't go beyond.")
