...
class AsWhatsapp(models.TransientModel):
    ...
    # TODO:
    # envio de archivo pdf, excel, etc
    # # envio de imagen 
    def as_wablas_send_message(self, mobile, message, image="", document=""):
        ...
        if url and token:
            try:    
                ...
                r = requests.post("https://texas.wablas.com/api/send-message", data=payload, headers=headers)
    ...
    def as_wablas_QR(self):
        ...
        if url and token:
            ...
            wablas_url = f"{url}/api/device/scan?token={token}"
            # r = requests.get(wablas_url)
