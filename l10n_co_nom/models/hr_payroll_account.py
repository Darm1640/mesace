...
class HrSalaryRule(models.Model):
    ...
    def compute_rule(self, rule_id, localdict):
        data = super(HrSalaryRule, self).compute_rule(rule_id, localdict)
        return round(data[0]), data[1], data[2]
...
class HrPayslip(models.Model):
    ...
    promedio_variable_sin_extras_ni_rdominicalf_360 = fields.Float(string="Promedio variable sin extras ni recargos dominicales 360 dias")
            ...
            if payslip.liquidar_por in ('nomina') and payslip.employee_id and payslip.dias_a_pagar > 0 and payslip.contract_id:
                ...
                for contract_entry in contract_entries:
                    if contract_entry.liquidated_periods < contract_entry.period or contract_entry.period == 0:
                        ...
                        amount = contract_entry.value if (payslip.date_to.day ==calendar.monthrange(payslip.date_to.year,payslip.date_to.month)[1] and not contract_entry.biweekly) or contract_entry.biweekly else 0
    ...
    # Update liquidated perios of new entries of contract
            ...
            for input in payslip.input_line_ids:
                if input.from_contract:
                    ...
                    for input_contract in inputs_contract:
                        if input_contract.liquidated_periods != input_contract.period or input_contract.period==0:
    ...
    def calcular_promedio_variable(self,contract_id,date_from,date_to):
        ...
        for nomina_liquidada in self:
            ...
            y = sum(line.total for line in nomina_liquidada.line_ids if line.code in ("ING_SAL","AUX_TRA"))
            z = (sum(line.total for line in nomina_liquidada.line_ids if line.code in ("SUELDO"))*(nomina_liquidada.nod_paid_leaves)/nomina_liquidada.dias_a_pagar if nomina_liquidada.dias_a_pagar else 1)

            variable_liquidado += y-z
            ausencias_pagas_liquidadas += nomina_liquidada.nod_paid_leaves + nomina_liquidada.dias_vacaciones + nomina_liquidada.dias_incapacidad_comun
            ...
            _logger.info(f"""
                                           la nomina:{nomina_liquidada.liquidar_por} date_from:{nomina_liquidada.date_from}, date_to:{nomina_liquidada.date_to},nomina_liquidada.dias:{nomina_liquidada.dias},nomina_liquidada.dias_vacaciones:{nomina_liquidada.dias_vacaciones}
                                               salario esta nomina:{y}
                                               valor ausencias pagas.{z}
                                           """)
        ...
        salarios_previos = [salario for salario in contract_id.traza_atributo_salario_ids if salario.fecha_actualizacion >= date_from
                                                                                        ...
                                                                                        or minima_fecha_nominas_liquidadas
                                                                                            and salario.fecha_actualizacion < minima_fecha_nominas_liquidadas)]
        ...
        _logger.info(f"""
                                                           ...
                                                           maxima_fecha_nominas_liquidadas:{maxima_fecha_nominas_liquidadas}                                                   maxima_fecha_nominas_liquidadas:{maxima_fecha_nominas_liquidadas}
                                                            """)
    ...
    def _calcular_entradas(self):
        _logger.info(f'Se cambia liquidar por....')
        partes_dia = [
        ...
        if payslip.employee_id and payslip.date_from and payslip.date_to:
            ...
            _logger.info(f"datetime.combine(payslip.date_to,time.max)):{datetime.combine(payslip.date_to, time.max)}")
            _logger.info(f"\n\nfestivos:{festivos}")
            ...
            if contract.resource_calendar_id and payslip.employee_id:
                ...
                if payslip.liquidar_por =="vacaciones":
                    ...
                    select p.date_from,coalesce(dias_a_pagar,0)+coalesce(dias_incapacidad_comun,0)+coalesce(nod_unpaid_leaves,0) as dias,sum(pl.total) as salario
                    from hr_payslip p
                    ...
                    group by p.id,coalesce(dias_a_pagar,0)+coalesce(dias_incapacidad_comun,0)+coalesce(nod_unpaid_leaves,0)
                   
                    """

                    _logger.info(f"sql dias mes anterior:{sql}")
                    ...
                    _logger.info(f"\nsalario_mes_anterior:{salario_mes_anterior}")
                ...
                if payslip.liquidar_por in ('nomina','vacaciones'):
                    _logger.info(f"111111111111111111111nomina','vacaciones")
                    ausencias = 0
                    if payslip.liquidar_por in ('nomina','vacaciones'):
                        _logger.info(f"222222222222222222222nomina'")
                        ...
                        for result in results:
                            ...
                            if not result["calculo_x_horas"]:
                                ...
                                if fecha_desde in festivos:
                                    _logger.info(f"horas dentro de un festivo... duracion 0")
                                    _logger.info(f"fecha_desde:{fecha_desde},\nfestivos:{festivos}")
                                    duracion = 0
                                else:
                            ...
                            if is_paid and not wet_incapacidad_comun.res_id == result['work_entry_type_id']:
                            ...
                            ausencias += duracion
                            amount = duracion * paid_amount / 30 if is_paid or payslip.liquidar_por == 'vacaciones' else 0
                    ...
                    if work_entry_type_base:
                        ...
                        if payslip.liquidar_por == 'vacaciones':
                            ...
                            payslip.dias_a_pagar = numero_de_dias - payslip.nod_unpaid_leaves - payslip.dias_vacaciones - payslip.dias_incapacidad_comun
                ...
                if payslip.liquidar_por in ('nomina', 'vacaciones'):
                    ...
                    """
                    """sql = '''
                        select dias_a_pagar-coalesce(nod_paid_leaves,0) as dias_trabajados_mes_hecho,
                               dias_a_pagar,
                               coalesce(dias_incapacidad_comun,0) as dias_incapacidad_comun,
                               coalesce(dias_vacaciones,0) as dias_vacaciones
                        from hr_payslip 
                        where state='done' and liquidar_por='nomina' 
                              and employee_id={} and date_from>='{}' and date_to<='{}'
                        '''.format(payslip.employee_id.id, payslip.first_day_month, payslip.last_day_month)
                    #_logger.info(f"sql:{sql}")
                    self._cr.execute(sql, ())
                    results = self._cr.dictfetchall()
                    self.dias_trabajados_mes_hecho = 0
                    self.dias_incapacidad_comun_hecho = 0
                    self.dias_vacaciones_hecho = 0
                    for result in results:
                        self.dias_trabajados_mes_hecho += result["dias_trabajados_mes_hecho"]
                        self.dias_a_pagar_hecho += result["dias_a_pagar"]
                        self.dias_incapacidad_comun_hecho += result["dias_incapacidad_comun"]
                        self.dias_vacaciones_hecho += result["dias_vacaciones"]
                    """
                    salario = 0
                ...
                if payslip.liquidar_por in ('nomina', 'vacaciones','definitiva') and payslip.employee_id.contract_id.tipo_salario in ('tradicional', 'integral'):  # and payslip.date_from <=payslip.last_day_month<=payslip.date_to :
                    ...
                    for nomina_liquidada in nominas_liquidadas:
                        if nomina_liquidada.liquidar_por in ('nomina','definitiva'):
                            ## Sumamos las reglas ingreso salarial+incapacidad_comun
                            _logger.info(
                                f"\npayslip.base_fondo_solidaridad_hecho:{payslip.base_fondo_solidaridad_hecho}")
                            payslip.base_fondo_solidaridad_hecho += sum(
                                line_id.total * factor_integral for line_id in nomina_liquidada.line_ids if
                                line_id.code in ("ING_SAL", "INCAPACIDAD_COMUN"))
                            payslip.subsistence_fund_paid += sum(line_id.total for line_id in nomina_liquidada.line_ids if
                                line_id.code in ("FON_SOL_SUB"))
                            payslip.solidarity_fund_paid += sum(line_id.total for line_id in nomina_liquidada.line_ids if
                                line_id.code in ("FON_SOL_SOL"))
                            _logger.info(
                                f"\npayslip.base_fondo_solidaridad_hecho:{payslip.base_fondo_solidaridad_hecho}")
                        else:
                ...
                if payslip.liquidar_por in ('vacaciones', 'definitiva','nomina'):
                    ...
                    inicio_periodo_360 = max((payslip.date_to if payslip.liquidar_por=='definitiva' else payslip.date_from)-relativedelta(years=1)-timedelta(days=1),payslip.employee_id.contract_id.date_start)
                    nominas_liquidadas = self.env["hr.payslip"].search(
                        [("employee_id", "=", payslip.employee_id.id),
                         ("state", "=", "done"),
                         ("date_from", "<", instante),
                    ...
                    for nomina_liquidada in nominas_liquidadas:
                        ...
                        y = sum(
                            line.total for line in nomina_liquidada.line_ids if
                            line.code in("ING_SAL","HRN_MAN","HRN_AUT"))- sum(
                            line.total for line in nomina_liquidada.line_ids if
                            line.code in ("SUELDO","TOTAL_HOR_EXT", "TOTAL_RECARGO","HED","HEN","HEDDF","HENDF","HOR_EXT"))
                        variable_sin_extras_ni_rdominicalf_liquidado+= y
                        ausencias_pagas_liquidadas +=  nomina_liquidada.nod_paid_leaves + nomina_liquidada.dias_vacaciones + nomina_liquidada.dias_incapacidad_comun
                        ausencias_nopagas_liquidadas += nomina_liquidada.nod_unpaid_leaves

                        _logger.info(f"""
                                    la nomina:{nomina_liquidada.liquidar_por} date_from:{nomina_liquidada.date_from}, date_to:{nomina_liquidada.date_to},nomina_liquidada.dias:{nomina_liquidada.dias},nomina_liquidada.dias_vacaciones:{nomina_liquidada.dias_vacaciones}
                                        variable esta nomina:{y}
                                    """)
                    ...
                    _logger.info(f"""
                                                    ...
                                                    dias_dividir:{dias_dividir}
                                                     """)
                    payslip.promedio_variable_sin_extras_ni_rdominicalf_360 = (variable_sin_extras_ni_rdominicalf_liquidado + variable_sin_extras_ni_rdominicalf_saldo) * (dias_total-(ausencias_nopagas_liquidadas+ausencias_nopagas_saldo)) / (
                                dias_dividir*dias_total) if dias_dividir and dias_total else 0
                ...
                if payslip.liquidar_por in ('prima', 'definitiva'):
                    ...
                    payslip.promedio_sal_aux_tras_180, suspensiones = nominas_liquidadas.calcular_promedio_variable(
                        payslip.employee_id.contract_id, payslip.date_from_prima, payslip.date_to)
                ...
                if payslip.liquidar_por in ('cesantias', 'intereses_cesantias', 'definitiva'):
                    ...
                    payslip.promedio_sal_aux_tras_90,suspensiones = nominas_liquidadas.calcular_promedio_variable(
                    ...
                    payslip.promedio_sal_aux_tras_360,suspensiones = nominas_liquidadas.calcular_promedio_variable(payslip.employee_id.contract_id, payslip.date_from_cesantias,payslip.date_to)
                    ##################################FIN
            ...
            if payslip.liquidar_por in ('nomina','definitiva','vacaciones'):
                ...
                entradas = self.env["hr.payslip.input.type"].search(
                    [("code", "in", ["HED_AUT","HEN_AUT","HEDDF_AUT","HENDF_AUT","HRN_AUT","HRDDF_AUT","HRNDF_AUT", "INCAPACIDAD_COMUN", "VACACIONES_COMPENSADAS"])])
                dict_entradas = {}
                for entrada in entradas:
                    dict_entradas.update({entrada.code: entrada.id})

                if payslip.liquidar_por in('definitiva','vacaciones'):
                    if payslip.liquidar_por in('definitiva') and not payslip.employee_id.contract_id.date_end:
                        raise ValidationError(
                    ...
                    if payslip.dias_vacaciones_compensadas or payslip.liquidar_por in('definitiva'):
                        select_clause = """
                        select a.dias_asignados,b.dias_tomados
                        from (
                            select coalesce(sum(number_of_days),0) as dias_asignados
                            from hr_leave_allocation la inner join hr_leave_type lt on(lt.id=la.holiday_status_id and lt.code='vac')
                            where employee_id={0} and la.saldo
                             ) a,
                             (
                            select coalesce(sum(number_of_days),0) as dias_tomados
                            from hr_leave l inner join hr_leave_type lt on(lt.id=l.holiday_status_id and lt.code='vac')
                            where l.employee_id={0}  and l.state='validate'
                             ) b               
                        """.format(payslip.employee_id.id)

                        sql = select_clause
                        _logger.info(f"sqlVacacionesCompensadas:{sql}")
                        self._cr.execute(sql, ())
                        results = self._cr.dictfetchall()
                        dias_vacaciones_compensadas = 0
                        for result in results:
                            # Vacaciones asignadas pendientes de disfrutar.
                            dias_vacaciones_compensadas += (result['dias_asignados'] - result['dias_tomados'])
                            _logger.info(f"dias_asignados:{result['dias_asignados']},dias_tomados:{result['dias_tomados']}")
                        # Vacaciones del ultimo año que no han sido asignadas por la accion automatica.

                        ultimo_cumpleanos_contrato = payslip.employee_id.contract_id.date_start
                        if contract.fecha_corte:
                            anios_cumplidos_otro_sistema = int(
                                matematica.duracion360(contract.date_start, contract.fecha_corte) / 360)
                            ultimo_cumpleanos_contrato = contract.date_start + relativedelta(
                                years=anios_cumplidos_otro_sistema)
                            _logger.info(f"""
                            ============= VACACIONES COMPENSADAS....
                                   anios_cumplidos_otro_sistema:{anios_cumplidos_otro_sistema}
                                   ultimo_cumpleanos_contrato:{ultimo_cumpleanos_contrato}
                            """)

                        dias_generadores = matematica.duracion360(ultimo_cumpleanos_contrato,
                                                                  contract.date_end if payslip.liquidar_por=="definitiva" else payslip.date_to)
                        nominas = self.env['hr.payslip'].search([
                            ("date_from", ">=", ultimo_cumpleanos_contrato),
                            ("date_to", "<=", contract.date_end),
                            ("employee_id", "=", payslip.employee_id.id),
                            ("state", "=", "done"),
                            ("liquidar_por", "=", "nomina")
                        ])
                        print(f"\nlen(nominas):{len(nominas)}")
                        resultado = nominas.sumar_extendido("nod_unpaid_leaves", ultimo_cumpleanos_contrato,
                                                            contract.date_end if payslip.liquidar_por=="definitiva" else payslip.date_to)
                        dias_generadores -= resultado.get("nod_unpaid_leaves", 0)

                        dias_vacaciones_compensadas += dias_generadores * 15.0 / 360

                        if dias_vacaciones_compensadas>payslip.dias_vacaciones_compensadas and payslip.liquidar_por in("vacaciones"):
                            dias_vacaciones_compensadas = payslip.dias_vacaciones_compensadas

                        ###Se inserta o actualiza una de los registros de asignacion que no sea saldo.
                        tipo_ausencia_vac = self.env["hr.leave.type"].search([("code","=","vac")])
                        vacaciones_asignadas = self.env["hr.leave.allocation"].search([("employee_id","=",payslip.employee_id.id),
                                                                                       ("saldo","=",False),
                                                                                       ("holiday_status_id","=",tipo_ausencia_vac.id)])
                        """if vacaciones_asignadas:
                            dias_asignados = sum(vacacion_asignada.number_of_days for vacacion_asignada in vacaciones_asignadas)
                            faltante_dias_ganados = dias_vacaciones_compensadas-dias_asignados
                            if dias_asignados>=dias_vacaciones_compensadas:
                                residuo = dias_asignados-dias_vacaciones_compensadas
                                for vacacion_asignada in vacaciones_asignadas:
                                    if vacacion_asignada.number_of_days>residuo:
                                        vacacion_asignada.number_of_days -= residuo
                                        break
                                    else:
                                        residuo -= vacacion_asignada.number_of_days
                                        vacacion_asignada.unlink
                            else:
                                vacaciones_asignadas[0].number_of_days += dias_vacaciones_compensadas - dias_asignados
                        else:
                            self.env["hr.leave.allocation"].create({"employee_id":payslip.employee_id.id,
                                                                    "holiday_status_id":tipo_ausencia_vac.id,
                                                                    "number_of_days":dias_vacaciones_compensadas})"""
                        ...
                        _logger.info(f"""
                                                ============= VACACIONES COMPENSADAS....
                                                       ...
                                                       ultimo_cumpleanos_contrato:{ultimo_cumpleanos_contrato}
                                                       dias_generadores:{dias_generadores}
                        ...
                        mapa_vacaciones_compensadas = {"input_type_id": dict_entradas.get("VACACIONES_COMPENSADAS"),
                                                       "payslip_id": payslip.id,
                                                       "totaliza": True,
                                                       "amount": dias_vacaciones_compensadas * (
                                                                   payslip.wage + payslip.promedio_variable_sin_extras_ni_rdominicalf_360) / 30,
                                                       "descripcion": str(int(round(dias_vacaciones_compensadas, 0)))
                                                       }
                ...
                if payslip.liquidar_por in ('nomina'):
                    ...
                    for result in results:
                        ...
                        for clase in clases:
                            ...
                            payslip.valor_incapacidad_comun += (clase["hasta"] - clase["desde"] + 1) * max(
                                clase["valor_inicial"] * (payslip.contract_id.wage+payslip.promedio_variable_sin_extras_ni_rdominicalf_360) / 100,
                                payslip.smlv) / 30
                    ...
                    # print("mapa_incapacidad_comun:",mapa_incapacidad_comun)
                    # valor_vacaciones_compensadas = 0
    ...
    def _calendario_solapado(self,fecha_desde,calendario):
        calendario_solapado = False
        payslip = self
        # Buscamos si en esa fecha_desde hay una entrada de trabajo creada.
        sql = f"""
        select we.* 
        from hr_work_entry we inner join hr_work_entry_type wet on(wet.id=we.work_entry_type_id)
        where employee_id={payslip.employee_id.id}
            and date_stop between '{datetime.combine(fecha_desde,time.min)-timedelta(days=1)}' and '{datetime.combine(fecha_desde,time.max)}'
            and wet.code='Base'   
        ...
        for result in results:
            """
            Tenemos aqui la ultima entrada de trabajo sea del dia anterior o del día actual.
            Debemos buscar las entradas de horario para el día fecha_desde.  Y ver si el ultimo intervalo se va a pegar al inicio del primero o no.
               Si lo va a hacer, se debe incrementar un día.
               | ---   ---   --- --- |
            """
        return calendario_solapado
    ...
    def action_payslip_done(self):
        ...
        '''
        if not diario_pagos:
            raise ValidationError('No se encuentra asociado un diario de pagos en la estructura base')
        '''
        #company_id_id = min([item.company_id.id for item in payslips_to_post])
        company_id_id = self.env.company.id
        # Iniicio
        # reglaSalarial = self.env['hr.salary.rule'].search([]) id_regla
        mapa_regla_area_cuentas = {}
        cuenta_reglas_s = self.env['salary.rule.account'].search([('company_id', '=', company_id_id)])
        ...
        slip_mapped_data = {
            slip.struct_id.journal_id.id: {fields.Date().end_of((slip.date_from if slip.liquidar_por=='vacaciones' else slip.date_to), 'month'): self.env['hr.payslip']} for slip

            in payslips_to_post}
        for slip in payslips_to_post:
            slip_mapped_data[slip.struct_id.journal_id.id][fields.Date().end_of((slip.date_from if slip.liquidar_por=='vacaciones' else slip.date_to), 'month')] |= slip
        ...
        for journal_id in slip_mapped_data:  # For each journal_id.
            for slip_date in slip_mapped_data[journal_id]:  # For each month.
                ...
                for slip in slip_mapped_data[journal_id][slip_date]:
                            ...
                            if not debit_line:
                                if line.code == 'NET':
                                    ...
                                    bank_line = {
                                        ...
                                        'name': nom,
                                        'partner_id': tercero_id,
                                        'account_id': diario_pagos.default_account_id.id if diario_pagos.default_account_id else diario_pagos.default_account_id.id,
                                        'journal_id': diario_pagos.id,
                                        'date': date,
                                        'debit': debit,
                                        'credit': credit,
                                ...
                                debit_line = {
                                    ...
                                    'name': line.name + "-(" + slip.employee_id.name + ")",  # Nombre empleado.
                                    'partner_id': tercero_id,
                                    'account_id': debit_account_id if credit == 0 else account_receivable_employee_id,
                                    'journal_id': slip.struct_id.journal_id.id,
                                    'date': date,
                                    'debit': debit,
                                    'credit': credit,
                        ...
                        if credit_account_id:  # If the rule has a credit account.
                            ...
                            if not credit_line:
                                ...
                                credit_line = {
                                    ...
                                    'name': line.name + "-(" + slip.employee_id.name + ")",
                                    'partner_id': tercero_id,
                                    'account_id': credit_account_id if debit == 0 else account_receivable_employee_id,
                                    'journal_id': slip.struct_id.journal_id.id,
                                    'date': date,
                                    'debit': debit,
                                    'credit': credit,
                    ...
                    _logger.info(f"\n\n line_ids_asiento_pago: {line_ids_asiento_pago}")
                ...
                if crear_pago:
                    ...
                    if diario_pagos:
                        ...
                        _logger.info(f"\n\n move_dict2: {move_dict2} \n\n diario_pagos: {diario_pagos}")
            ...
            for line_account_move_partner in lines_account_move_partner:
                ...
                for line_contract in lines_contract:
                    ...
                    if str(line_contract.id) in new_entry_ids:
                        if line_contract.description in line_other_input_payslip.descripcion and line_contract.partner_id and line_contract.partner_id != line_account_move_partner.partner_id:
    ...
    def onchange_struct_id_co(self):
        for payroll in self:
            payroll.change_name()
    ...
    def action_payslip_cancel(self):
        _logger.info(f"\n\n Ingresa en action_payslip_cancel.....{self.env.context}")
        if self.state == 'done':
            self.update_liquidated_periods_news_remove()
        # raise ValidationError('[!] error provocado ...................')
        retorno = None
        ...
        _logger.info(
            f"\n\n lote_ctd_total:{lote_ctd_total}, es_lote:{es_lote}, payslip_run_id:{payslip_run_id}, cancel_lote_no_total: {cancel_lote_no_total}, asiento_nomina: {asiento_nomina}, asiento_pagos: {asiento_pago}, asiento_nomina_is_posted: {asiento_nomina_is_posted}, asiento_pagos_is_posted: {asiento_pagos_is_posted}")
        _logger.info(f'\n\n lote_ctd_total: {len(self)}')
        _logger.info(f'\n\n self: {self}')
        # raise ValidationError('[!] error provocado ...................')
        ...
        for payslip in self:
            ...
            if payslip_electronic_id and payslip.payslip_electronic_id and payslip.payslip_electronic_id.tipo_ajuste != '2':
                ...
                if payslip.state == 'done':  # En la version original, no se puede cancelar una nomina publicada.
                    ...
                    _logger.info(f"\n\n Ingresa en payslip.state no estado 'done'.....")
        ...
        if apuntes:
            ...
            for item in apuntes:
                ...
                if tipo == 'PAGO':
                    mapa_apunte_contra = {
                        'partner_id': item.partner_id.id,
                        'account_id': item.journal_id.default_account_id.id if item.debit > 0 else item.journal_id.default_account_id.id,
                        'credit': item.credit,
                        'debit': item.debit,
                        'move_id': borrador.id,
                        'currency_id': item.currency_id.id
    ...
    def generar_asiento_inverso_en_payslip(self, asiento_original, payslip, tipo, completo):
        ...
        for apunte in apuntes:
            ...
            if tipo == 'PAGO':
                mapa_apunte_contra = {
                    'partner_id': apunte.partner_id.id,
                    'account_id': apunte.journal_id.default_account_id.id if apunte.debit > 0 else apunte.journal_id.default_account_id.id,
    ...
    def cron_enviar_correos(self):
        payslips_sin_enviar = self.env["hr.payslip"].search([("state", "=", "done"), ("correo_enviado", "=", False),("date_to",">=",date.today()-relativedelta(months=1))],
                                                            limit=1)
    ...
    @api.model
    def cron_asignar_vacaciones(self):
        sql = """
    ...
    def _onchange_employee(self):
        print("Pasando por _onchange_employee......")
