...
class HrPayslipElectronic(models.Model):
            ...
            try:
                ...
                try:
                    ...
                    sql = """select cp.id,p.code as codigo_pais,e.state_code as codigo_departamento,ci.city_code as codigo_municipio
                        ...
                        """.format(partner_id_empleador.postal_id2.name)
                    ...
                    if nomina_electronica.tipo_ajuste != '2' or len(payslip_ids)!=0:
                        for payslip_id in payslip_ids:
                            ...
                            for line_id in payslip_id.line_ids:
                                ...
                                elif line_id.code == "RET_FUE" or line_id.code == "RET_FTE":
    ...
    """
        ...
        """
    @api.model
    def cron_creacion_xml(self):
        self._creacion_nominas_pendientes_mes_anterior()
        """
        B.  Filtro de lo que esta pendiente de enviarse o que el envio no haya pasado de manera exitosa.
        """
    ...
    """
    @api.model
    def _creacion_nominas_pendientes_mes_anterior(self):
        ...
        sql = """
                            ...
                            where p.state='done' and ((to_char(p.date_from,'yyyy-mm')=to_char(current_date-'1 month'::interval,'yyyy-mm')
                                  and p.liquidar_por in('nomina','vacaciones')) or (to_char(p.date_to,'yyyy-mm')=to_char(current_date-'1 month'::interval,'yyyy-mm')
                            ...
                            where company_id=company_id1 and employee_id=employee_id1 and state='done' and ((to_char(date_from,'yyyy-mm')=to_char(current_date-'1 month'::interval,'yyyy-mm')
                                  and liquidar_por in ('nomina','vacaciones')) or (to_char(date_to,'yyyy-mm')=to_char(current_date-'1 month'::interval,'yyyy-mm')
                            ...
                            where company_id=company_id1 and employee_id=employee_id1 and state='done' 
                                  and to_char(hr_payslip.create_date,'yyyy-mm')=to_char(current_date-'1 month'::interval,'yyyy-mm')
