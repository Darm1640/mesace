...
class HREmployee(models.AbstractModel):
    ...
    afc = fields.Integer(string='Valor aporte mensual a cuenta AFC',
                         help='Aporte a la cuenta de Ahorro Fomento a la Construcción')
    avc = fields.Integer(string='Valor aporte mensual a cuenta AVC',
                         help='Aporte a la cuenta de Ahorro Voluntario Contractual')
    ...
    exento_transporte = fields.Boolean(
        string="Exento subsidio de transporte(Trasportado por empresa o menos de un kilometro)")
    ...
    #         #else:
        ...
        #             'Faltan los siguientes campos:{}'.format(note))
        # return created
...
class HREmployee1(models.Model):
    ...
    def generate_work_entries(self, date_start, date_stop):
        ...
        '''
        not_work_entries_view_gantt = self.env['ir.config_parameter'].sudo(
        ).get_param('payroll.not_work_entries_view_gantt')
        if not_work_entries_view_gantt:
            res = super(HREmployee1, self).generate_work_entries(
