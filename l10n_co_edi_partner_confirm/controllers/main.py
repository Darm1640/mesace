...
class Main(http.Controller):
    ...
    @http.route("/l10n_co_edi_partner_confirm/accept_dian_invoice", type='http', auth='none', csrf=False)
    def accept_dian_invoice(self, dian_document):
        rec_dian_document = request.env['account.move'].sudo().search([('ei_uuid','=', dian_document)])
    ...
    @http.route("/l10n_co_edi_partner_confirm/reject_dian_invoice", type='http', auth='none', csrf=False)
    def reject_dian_invoice(self, dian_document):
        #rec_dian_document = request.env['account.move'].sudo().browse(int(dian_document))
        rec_dian_document = request.env['account.move'].sudo().search([('ei_uuid','=', dian_document)])
        if not rec_dian_document:
            return u'<html><body><h2>Este documento no existe</h2></body></html>'
        if rec_dian_document.date_email_acknowledgment:
            ...
            <html>
                <body>
                    <form action="/l10n_co_edi_partner_confirm/reject_dian_invoice_reason">
                        <table>
    ...
    @http.route("/l10n_co_edi_partner_confirm/reject_dian_invoice_reason", type='http', auth='none', csrf=False)
